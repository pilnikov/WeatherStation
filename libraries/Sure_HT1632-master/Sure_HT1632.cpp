...
/*
   ...
   functions go here:
*/
HT1632Class::HT1632Class(uint8_t pinCLK, uint8_t pinCS1, uint8_t pinWR, uint8_t pinDATA) : Adafruit_GFX(32, 16) {
...
void HT1632Class::drawImage(const byte * img, uint8_t width, uint8_t height, int8_t x, int8_t y, int img_offset) {
  ...
  while (src_x < width) {
    ...
    while (src_y < height) {
      ...
      copyInNextStep = min(int(copyInNextStep), int(height - src_y));
...
void HT1632Class::drawPixel(int16_t x, int16_t y, uint16_t channel) 
{
  if ( x < 0 || x > OUT_SIZE || y < 0 || y > COM_SIZE ) return;
  
  if (channel < 2) mem[channel][GET_ADDR_FROM_X_Y(x, y)] |= GET_BIT_FROM_Y(y);
  if (channel == 2) 
  {
	mem[0][GET_ADDR_FROM_X_Y(x, y)] |= (GET_BIT_FROM_Y(y));
    mem[1][GET_ADDR_FROM_X_Y(x, y)] |= (GET_BIT_FROM_Y(y));
  }
...
void HT1632Class::drawPartChar(int16_t x, int16_t y, unsigned char c, uint8_t color, uint8_t bg, int8_t size)
{
  ...
  for (uint8_t i = 0; i < 5; i++ )
	{ // Char bitmap = 5 columns
		...
		for (uint8_t j = a; j < b; j++)  
		{
			if (size > 0) 
			{
				if (line & 1 << (j + 7 - size)) setPixel(x + i, y + j, color);
			}
			else 
			{
				if (line & 1 << (j + size)) setPixel(x + i, y + j, color);
			}
		}
	} 
