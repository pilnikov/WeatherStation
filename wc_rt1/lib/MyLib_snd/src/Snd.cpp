...
void Synt::play(uint8_t snum, uint8_t out, bool _play, bool pola)
{
  ...
  static bool set_up, is_played;

  if (_play & !is_played)
  {
    char textBuffer[440];
    strcpy_P(textBuffer, (char*)pgm_read_dword(&(songs[snum])));
    p = textBuffer;
...
//#ifdef _debug
		DBG_OUT_PORT.print(F("pause: "));
		DBG_OUT_PORT.println(duration);
