  ...
  public:
    ...
    roll_string_cf(uint8_t &num, uint8_t _max, main_cfg_t mcf, snr_data_t snr, wf_data_t wf, wf_data_t wfc,
                   rtc_time_data_t rt, rtc_alm_data_t rta, String local_ip, uint8_t c_br, char out[], bool cur_cli, String newsClient),  // Формирует контент для бегущей строки
                                   ...
                                   scroll_string_ran_selector(uint8_t vdrv_t, uint8_t dsp_t, bool slow_mode, bool& _end, char* in_st, byte* out),  // Формирует бегущую строку
                                   roll_string_part_sw(uint8_t &num, uint8_t _max, main_cfg_t mcf, snr_data_t snr, wf_data_t wf,
                                       wf_data_t wfc, rtc_time_data_t rt, rtc_alm_data_t rta, String local_ip, uint16_t c_br, bool cli, String ns,
                                       uint8_t &ni, bool &end, char *st1, byte *screen),  // Перезапись (инициализация) бегущей строки

                                   v_scroll_all(uint8_t, byte *);  // запуск вертикального скроллинга для всех изменившихся позиций

    bool symbol_time_part_view(bool use_pm, bool end_run_st, bool rus_lng, uint8_t dsp_t,
                               rtc_time_data_t rtd, rtc_alm_data_t rta, byte *out);  // Формирует часть фрейма с отображением текущего времени на символьных (матричных и LCD1602) дисплеях
  private:
...
class MyDspHW {
  ...
  private:
    ...
    tm1637_init(byte, byte),
                ...
                ht1632_ramFormer(byte *, const uint8_t, const uint8_t, uint8_t),
