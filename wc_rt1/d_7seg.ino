...
scr_buff_t ss_prep(uint8_t mod)
{
	...
	default://Текущее время
		uint8_t h = hour();
		// Do 24 hour to 12 hour format conversion when required.
		if (conf_data.use_pm && hour() > 12)
			h = hour() - 12;
		
		bstr = (h < 10 ? String(" ") : String("")) + String(h) + (minute() < 10 ? String("0") : String("")) + String(minute());

		if (ram_data.type_disp == 3 || ram_data.type_disp == 12)
		{
			bstr = bstr + (second() < 10 ? String("0") : String("")) + String(second());
		}
...
void sevenseg()
{
	...
	case 12:
		...
		if (conf_data.use_pm && hour() > 12)
			h = hour() - 12;
		bstr = (h < 10 ? String(" ") : String("")) + String(h) + (minute() < 10 ? String("0") : String("")) + String(minute());
		buf2 = f_dsp.prn7(bstr);
		buf2.dig[2] |= blinkColon << 7;
