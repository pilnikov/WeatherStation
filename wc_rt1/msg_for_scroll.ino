...
String pr_str(uint8_t num, conf_data_t cf, snr_data_t sn, wf_data_t wf, wf_data_t wfc, rtc_data_t rt, String local_ip, uint8_t c_br)
{
  ...
  size_buf = snprintf_P(buf, 10, PSTR(" String "));
  ...
  if (!cf.rus_lng)
  {
    ...
    {
      case 1:
        size_buf = snprintf_P(buf, 70, PSTR(" Today is %S %d %S %d"), sdne[rt.wday - 1], rt.day, smne[rt.month - 1], rt.year);
        break;

      case 2:
        if (sn.t1 > -99 && sn.t1 < 99)
        {
          size_buf  = snprintf_P(buf , 30, PSTR(" %s %d%cC %d%% "), cf.ch1_name, sn.t1, grad, sn.h1);
        }
        if (sn.t2 > -99 && sn.t2 < 99)
        {
          size_buf1 = snprintf_P(buf1, 30, PSTR(" %s "), cf.ch2_name);
          ...
          size_buf1 = snprintf(buf1, 30, PSTR("%d%cC %d%%"), sn.t2, grad, sn.h2);
          strncat(buf, buf1, size_buf1);
        }
        if (sn.t3 > -99 && sn.t3 < 99)
        {
          size_buf1 = snprintf_P(buf1, 30, PSTR(" %s %d%cC %d%% "), cf.ch3_name, sn.t3, grad, sn.h3);
          strncat(buf, buf1, size_buf1);
        }
        if (sn.p > 700)
        {
          size_buf1 = snprintf_P(buf1, 60, PSTR(" preesure %dmm m"), sn.p);
          strncat(buf, buf1, size_buf1);
        }
        break;
      ...
      case 3:
        ...
        {
          case 1:
            wf.temp_min > -99 ? size_buf = snprintf_P(buf, 255, PSTR(" Weather forecast from GM on %S %d %S:temp from %d to %d%cC wind %S %d - %dm/s %S humid. %d%% press %dmm.rt.st"),
          ...
          case 2:
            ...
            wf.temp_min > -99 ? size_buf = snprintf_P(buf, 255, PSTR(" Weather forecast from OWM on %d %S:%S temp from %d to %d%cC wind %S %dm/s humid %d%% press %dmm.rt.st"),
                                           wf.day, smnr[wf.month - 1], buf1,
                                           wf.temp_min, wf.temp_max, grad, swnr[wf.wind_dir], wf.wind_min, wf.hum_min, wf.press_min)
                                           : size_buf = snprintf_P(buf, 255, PSTR(" Forecast not received! check the setting "));
            break;
          default:
            size_buf = snprintf_P(buf, 50, PSTR(" Forecast is disabled! "));
            break;
        }
        break;

      case 4:
        (ala_t > cur_t) ? size_buf = snprintf_P(buf, 120, PSTR(" Alarm from %2d h. %2d min. on %2d:%02d. Curr brigthness: %2d Your IP:%s"), ala_h, ala_m, rt.a_hour % 100, rt.a_min, cur_br, _buff)
                                     : size_buf = snprintf_P(buf, 110, PSTR(" Alarm is not set. Curr brigthness:%2d Your IP:%s"), cur_br, _buff);
        break;
      default:
        size_buf = snprintf_P(buf, 10, PSTR(" String "));
        break;
  ...
  else
  {
    ...
    {
      case 1:
        size_buf = snprintf_P(buf, 70, PSTR(" Сегодня %S %d %S %dг."), sdnr[rt.wday - 1], rt.day, smnr[rt.month - 1], rt.year);
        break;
      ...
      case 2:
        ...
        if (sn.t2 > -99 && sn.t2 < 99)
        {
          ...
          if (cf.use_pp == 2)
          {
            ...
            strncpy(buf1, wf.descript.c_str(), size_buf1);
            strncat(buf, buf1, size_buf1);
            size_buf1 = snprintf_P(buf1, 50, PSTR(" ветер %S %dм/с "), swnr[wfc.wind_dir], wfc.wind_min);
            strncat(buf, buf1, size_buf1);
          }
        ...
        if (sn.p > 700)
        {
          size_buf1 = snprintf_P(buf1, 60, PSTR(" давление %dмм рт. ст."), sn.p);
          strncat(buf, buf1, size_buf1);
        }
        break;
      ...
      case 3:
        ...
        {
          case 1:
            wf.temp_min > -99 ? size_buf = snprintf_P(buf, 255, PSTR(" Прогноз погоды от GM на %S %d %S:температура от %d до %d%cC ветер %s %d - %dм/с %S oтн. влажность %d%% давление %dмм.рт.ст"),
                                           stdr[wf.tod], wf.day, smnr[wf.month - 1],
                                           wf.temp_min, wf.temp_max, grad, swnr[wf.wind_dir], wf.wind_max, wf.wind_min, sprcr[wf.prec],
                                           wf.hum_max, wf.press_max) : size_buf = snprintf_P(buf, 255, PSTR(" Данные по прогнозу погоды не получены - проверьте настройки "));
            break;
          case 2:
            size_buf1 = wf.descript.length() + 1;
            strncpy(buf1, wf.descript.c_str(), size_buf1);

            wf.temp_min > -99 ? size_buf = snprintf_P(buf, 255, PSTR(" Прогноз погоды от OWM на %d %S:%S температура от %d до %d%cC ветер %S %dм/с oтн.влажность %d%% давление %dмм.рт.ст"),
                                           wf.day, smnr[wf.month - 1], buf1,
                                           wf.temp_min, wf.temp_max, grad, swnr[wf.wind_dir], wf.wind_min, wf.hum_min, wf.press_min)
                                           : size_buf = snprintf_P(buf, 255, PSTR(" Данные по прогнозу погоды не получены - проверьте настройки"));
            break;
          default:
            size_buf = snprintf_P(buf, 50, PSTR(" Прогноз погоды отключен! "));
            break;
        }
        break;

      case 4:
        (ala_t > cur_t) ? size_buf = snprintf_P(buf, 120, PSTR(" Будильник зазвонит через %2d ч. %2d мин. в %2d:%02d. Текущая яркость: %2d Ваш IP:%s"), ala_h, ala_m, rt.a_hour % 100, rt.a_min, cur_br, _buff)
                                     : size_buf = snprintf_P(buf, 110, PSTR(" Будильник не установлен. Текущая яркость:%2d Ваш IP:%s"), cur_br, _buff);
        break;
      default:
        size_buf = snprintf_P(buf, 15, PSTR(" Строка "));
        break;
    }
  }
