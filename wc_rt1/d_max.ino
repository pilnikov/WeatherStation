...
void matrix_time()
{
  ...
  for (int i = 0; i < num; i++)
  {
    if (!(i == 0 && h < 9))
    {
      m7219 -> drawChar(digPos[i], 0, d[i], HIGH, LOW, 1);
    }
  }
...
# ifdef new_max
  ...
  for (int i = 0; i < num; i++)
  {
    if (!(i == 0 && h < 9))
    {
      m7219 -> drawPartChar(digPos[i],   0,        d[i], HIGH, LOW,   digtrans[i]); // набегает
      if (digtrans[i] != 7)
      {
        m7219 -> drawPartChar(digPos[i], 0, digoldig[i], HIGH, LOW, - digtrans[i]); // убегает
        /*
