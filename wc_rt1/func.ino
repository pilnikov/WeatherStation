...
void keyb_read()
{
  ...
  if (but0_pressed && !serv_act && millis() - setting_ms > 2000 && conf_data.type_thermo == 0) digitalWrite(LED_BUILTIN, LOW);  // Включаем светодиод
  if (but0_pressed &&  serv_act && millis() - setting_ms > 2000 && conf_data.type_thermo == 0) digitalWrite(LED_BUILTIN, HIGH); // Выключаем светодиод
