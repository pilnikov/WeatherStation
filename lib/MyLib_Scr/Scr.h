...
class MSG // Текстовые сообщения
{
  ...
  void 
	pr_str(uint8_t &num, uint8_t _max, main_cfg_t mcf, snr_data_t sn, wf_data_t wf, wf_data_t wfc, rtc_time_data_t rt, rtc_alm_data_t rta, String local_ip, uint8_t c_br, char out[], bool cur_cli, String newsClient),
	ili_time(bool, bool);
  bool
  ...
  private:
  uint8_t
	q_dig = 6; 	// ╨║╨╛╨╗╨╕╤З╨╡╤Б╤В╨▓╨╛ ╤Ж╨╕╤Д╤А ╨╜╨░ ╨┤╨╕╤Б╨┐╨╗╨╡╨╡
  bool
        old[6];
  const uint8_t 
	dposx[6] = {0, 6, 13, 19, 25, 29}; // ╨┐╨╛╨╖╨╕╤Ж╨╕╨╕ ╤Ж╨╕╤Д╤А ╨╜╨░ ╤Н╨║╤А╨░╨╜╨╡ ╨┐╨╛ ╨╛╤Б╨╕ x
  unsigned 
	char change[6];                         // ╤Г╨▒╨╡╨│╨░╤О╤Й╨░╤П ╤Ж╨╕╤Д╤А╨░
  uint16_t
	buffud[64];
