/*
MAX72xx - Library for using a MAX7219/7221 LED matrix controller
...
bool MAX72XX::clear(uint8_t buf)
{
  if (buf > LAST_BUFFER)
    return(false);
...
uint8_t MAX72XX::bitReverse(uint8_t b)
...
bool MAX72XX::copyColumn(uint8_t buf, uint8_t cSrc, uint8_t cDest)
{
  if (_hwDigRows) return(copyC(buf, cSrc, cDest));
  else return(copyR(buf, cSrc, cDest));
}

bool MAX72XX::copyRow(uint8_t buf, uint8_t rSrc, uint8_t rDest)
{
  if (_hwDigRows) return(copyR(buf, rSrc, rDest));
  else return(copyC(buf, rSrc, rDest));
}

bool MAX72XX::copyC(uint8_t buf, uint8_t cSrc, uint8_t cDest)
...
bool MAX72XX::copyR(uint8_t buf, uint8_t rSrc, uint8_t rDest)
// Src and Dest are in pixel coordinates.
...
uint8_t MAX72XX::getColumn(uint8_t buf, uint8_t c)
{
  if (_hwDigRows) return(getC(buf, c));
  else return(getR(buf, c));
}

uint8_t MAX72XX::getRow(uint8_t buf, uint8_t r)
{
  if (_hwDigRows) return(getR(buf, r));
  else return(getC(buf, r));
}

uint8_t MAX72XX::getC(uint8_t buf, uint8_t c)
...
uint8_t MAX72XX::getR(uint8_t buf, uint8_t r)
...
bool MAX72XX::setColumn(uint8_t buf, uint8_t c, uint8_t value)
{
  if (_hwDigRows) return(setC(buf, c, value));
  else return(setR(buf, c, value));
}

bool MAX72XX::setRow(uint8_t buf, uint8_t r, uint8_t value)
{
  if (_hwDigRows) return(setR(buf, r, value));
  else return(setC(buf, r, value));
}

bool MAX72XX::setC(uint8_t buf, uint8_t c, uint8_t value)
...
bool MAX72XX::setR(uint8_t buf, uint8_t r, uint8_t value)
...
bool MAX72XX::transform(uint8_t buf, transformType_t ttype)
{
  if (buf > LAST_BUFFER)
    return(false);
...
bool MAX72XX::transformBuffer(uint8_t buf, transformType_t ttype)
{
  uint8_t t[ROW_SIZE];
