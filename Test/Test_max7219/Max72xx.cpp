...
MAX72XX::MAX72XX(moduleType_t mod, uint8_t dataPin, uint8_t clkPin, uint8_t csPin, uint8_t numDevices):
...
MAX72XX::MAX72XX(moduleType_t mod, uint8_t csPin, uint8_t numDevices):
...
MAX72XX::MAX72XX(moduleType_t mod, SPIClass& spi, uint8_t csPin, uint8_t numDevices):
  _dataPin(0), _clkPin(0), _csPin(csPin),
  _hardwareSPI(true), _spiRef(spi), _maxDevices(numDevices), _updateEnabled(true)
#if defined(__MBED__) && !defined(ARDUINO)
  , _spi(SPI_MOSI, NC, SPI_SCK), _cs((PinName)csPin)
...
void MAX72XX::setModuleParameters(moduleType_t mod)
...
void MAX72XX::controlHardware(uint8_t dev, controlRequest_t mode, int value)
...
void MAX72XX::controlLibrary(controlRequest_t mode, int value)
...
bool MAX72XX::control(uint8_t startDev, uint8_t endDev, controlRequest_t mode, int value)
{
...
bool MAX72XX::control(uint8_t buf, controlRequest_t mode, int value)
...
void MAX72XX::flushBuffer(uint8_t buf)
